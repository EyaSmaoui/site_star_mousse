...
/* ─────────────────────────────────────────
   DATA — Matelas Medico Pillow (Gamme Pillow Top)
   Source : Fiche technique Super Siesta
───────────────────────────────────────── */
const PRODUCT = {
  name: "Matelas Medico Pillow",
  tag: "PILLOW TOP - MÉMOIRE DE FORME",
  tagColor: "#3498db", // Bleu pour la gamme Medico
  rating: 4.9,
  reviewCount: 142,
  ...
  // Plage de prix : 800.00 DT à 1,810.00 DT
  sizes: [
    { label: "90*190",   price: 800,   oldPrice: 995 }, // Prix promo fiche
  ...
  features: [
    "Hauteur Généreuse de 32 cm",
    "Couche de 8 cm de Mousse à Mémoire de Forme",
    "Technologie Ressorts Bonnells (Soutien ferme et durable)",
    "Supporte jusqu'à 110kg par personne",
    "Face Mi-ferme avec mousse épaisse et douce",
    "Réversible Été/Hiver",
    "Tissu Anti-acarien et Résistant au feu",
    "Garantie Constructeur : 9 ans",
  ],
};
  ...
  return (
    <>
      <style>{`
        ...
        .ssn-main-img { width: 100%; border-radius: 20px; background: #fff; padding: 25px; border: 1px solid #f1f5f9; box-shadow: 0 4px 20px rgba(0,0,0,0.02); }
        .ssn-thumbs { display: flex; gap: 12px; margin-top: 15px; }
        .ssn-thumb { width: 85px; height: 60px; border-radius: 10px; cursor: pointer; border: 2px solid transparent; transition: 0.2s; object-fit: cover; }
        .ssn-thumb.active { border-color: ${PRODUCT.tagColor}; }

        .ssn-badge { display: inline-block; padding: 6px 18px; background: ${PRODUCT.tagColor}; color: #fff; border-radius: 4px; font-size: 11px; font-weight: 700; letter-spacing: 0.5px; margin-bottom: 15px; }
        .ssn-title { font-family: 'Playfair Display', serif; font-size: 40px; color: #0f172a; margin-bottom: 10px; }
        
        .ssn-price-area { margin: 25px 0; padding: 25px; background: #f0f7ff; border-radius: 15px; border: 1px solid #e1effe; }
        .ssn-price-now { font-size: 38px; font-weight: 700; color: #1d4ed8; }
        ...
        .ssn-size-opt { padding: 14px 20px; border: 1px solid #e2e8f0; border-radius: 10px; background: #fff; cursor: pointer; font-weight: 600; transition: 0.3s; }
        .ssn-size-opt.active { background: #1a1a2e; color: #fff; border-color: #1a1a2e; }

        .ssn-order-box { background: #ffffff; padding: 30px; border-radius: 20px; border: 1px solid #e2e8f0; box-shadow: 0 10px 30px rgba(0,0,0,0.05); }
        .ssn-input { width: 100%; padding: 16px; margin-bottom: 12px; border: 1px solid #cbd5e1; border-radius: 10px; outline: none; }
        .ssn-btn-buy { width: 100%; padding: 18px; background: #1a1a2e; color: #fff; border: none; border-radius: 10px; font-weight: 700; cursor: pointer; font-size: 17px; transition: 0.3s; }
        .ssn-btn-buy:hover { background: #000; transform: translateY(-2px); }
        ...
        <div className="ssn-container">
          <div className="ssn-grid">
            ...
            <div className="ssn-gallery ssn-fade">
              <img src={PRODUCT.images[activeImg]} className="ssn-main-img" alt="Matelas Medico Pillow" />
              <div className="ssn-thumbs">
                {PRODUCT.images.map((img, i) => (
                  <img key={i} src={img} className={`ssn-thumb ${activeImg === i ? 'active' : ''}`} onClick={() => setActiveImg(i)} />
                ))}
              </div>
              <div style={{ marginTop: '20px', padding: '15px', background: '#eef2ff', borderRadius: '12px', color: '#3730a3', fontSize: '14px', textAlign: 'center', border: '1px solid #e0e7ff', fontWeight: '600' }}>
                🚛 Livraison gratuite sur le Grand Tunis
              </div>
            </div>
            ...
            <div className="ssn-fade" style={{ transitionDelay: "0.1s" }}>
              ...
              <p style={{ color: '#64748b', fontSize: '18px' }}>Gamme Souple : L'équilibre parfait entre confort et soutien</p>
              ...
              <span className="ssn-label">Choisir Dimensions :</span>
          ...
          <div className="ssn-tabs ssn-fade">
            ...
            <div style={{ padding: '40px 0', lineHeight: '1.8', color: '#475569', fontSize: '16px' }}>
              {activeTab === 'desc' ? (
                <div>
                  <p>Découvrez le <strong>Matelas Medico Pillow</strong> de Super Siesta, conçu pour allier confort et soutien durable. Sa couche de <strong>8 cm de mousse à mémoire de forme</strong> épouse parfaitement les contours de votre corps pour un soutien sur mesure qui minimise les points de pression.</p>
                  <p style={{ marginTop: '15px' }}>Avec une hauteur luxueuse de <strong>32 cm</strong> et une suspension à <strong>ressorts Bonnells</strong>, ce matelas conserve sa forme et son soutien au fil du temps. Il est idéal pour ceux qui souhaitent améliorer leur qualité de sommeil avec un produit d'excellence offrant un rapport qualité-prix remarquable.</p>
                </div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '15px' }}>
                  {PRODUCT.features.map((f, i) => (
                    <div key={i} style={{ padding: '15px', background: '#f8fafc', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '10px' }}>
